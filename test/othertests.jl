...
@testset "construction of $SV" for SV in (StridedView, UnsafeStridedView)
    @testset for T1 in (Float32, Float64, Complex{Float32}, Complex{Float64})
        ...
        @test_throws StridedTensorXD.ReshapeException SV(A5)
    ...
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        @testset for N = 2:6
            ...
            GC.@preserve R1 R2 R3 begin
                B1 = permutedims(SV(R1), Tuple(randperm(N)))
                B2 = permutedims(SV(R2), Tuple(randperm(N)))
                B3 = permutedims(SV(R3), Tuple(randperm(N)))
                A1 = convert(Array, B1)
...
@testset "views with $SV" for SV in (StridedView, UnsafeStridedView)
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        ...
        @test StridedTensorXD.offset(view(B,:,1:5,3,1:5)) == 2*stride(B,3)
    end
end
...
@testset "broadcast with $SV" for SV in (StridedView, UnsafeStridedView)
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        ...
        GC.@preserve R1 R2 R3 begin
            B1 = SV(R1)
            B2 = permutedims(SV(R2), Tuple(randperm(2)))
            B3 = permutedims(SV(R3), Tuple(randperm(3)))
            A1 = convert(Array, B1)
...
@testset "mapreduce with $SV" for SV in (StridedView, UnsafeStridedView)
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        ...
        R2 = rand(T, (10, 10, 10))
        R2c = copy(R2)
        @test StridedTensorXD._mapreducedim!(sin, +, identity, (10,10,10,10,10,10), (sreshape(SV(R2c),(10,1,1,10,10,1)), SV(R1))) ≈
            mapreduce(sin, +, R1; dims = (2, 3, 6)) .+ reshape(R2, (10,1,1,10,10,1))
        R2c = copy(R2)
        @test StridedTensorXD._mapreducedim!(sin, +, x->0, (10,10,10,10,10,10), (sreshape(SV(R2c),(10,1,1,10,10,1)), SV(R1))) ≈
            mapreduce(sin, +, R1; dims = (2, 3, 6))
        R2c = copy(R2)
        β = rand(T)
        @test StridedTensorXD._mapreducedim!(sin, +, x->β*x, (10,10,10,10,10,10), (sreshape(SV(R2c),(10,1,1,10,10,1)), SV(R1))) ≈
            mapreduce(sin, +, R1; dims = (2, 3, 6)) .+ β .* reshape(R2, (10,1,1,10,10,1))
        R2c = copy(R2)
        @test StridedTensorXD._mapreducedim!(sin, +, x->β, (10,10,10,10,10,10), (sreshape(SV(R2c),(10,1,1,10,10,1)), SV(R1))) ≈
            mapreduce(sin, +, R1; dims = (2, 3, 6), init = β)
        R2c = copy(R2)
        @test StridedTensorXD._mapreducedim!(sin, +, conj, (10,10,10,10,10,10), (sreshape(SV(R2c),(10,1,1,10,10,1)), SV(R1))) ≈
...
@testset "complete reductions with $SV" for SV in (StridedView, UnsafeStridedView)
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        ...
        R1 = permutedims(R1, (Tuple(randperm(6))...,))
        ...
        for op1 in (identity, conj, transpose, adjoint)
            ...
            for op2 in (identity, conj, transpose, adjoint)
                ...
                for op3 in (identity, conj, transpose, adjoint)
                    ...
                    α = 2 + im
                    β = 3 - im
                    StridedTensorXD.mul!(op3(B3), op1(B1), op2(B2), α, β)
