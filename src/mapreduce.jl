...
Apply function `f` to each element(s) in `A`, and then reduce the result using the binary
function `op` along dimensions of `A`. `dims` is a vector specifying the dimensions to
reduce, and the keyword argument `kw... = init` is the initial value to use in the
reductions. In general, it will be necessary to provide init to work with empty collections.
"""
Base.mapreduce(f, op, A::AbstractStridedView; dims=:, kw...) =
...
function _mapreduce_fuse!(@nospecialize(f), @nospecialize(op), @nospecialize(initop),
    ...
    @inbounds for i = length(dims):-1:2
        ...
        if merge
            dims = TupleLabXD.setindex(dims, dims[i-1]*dims[i], i-1)
            dims = TupleLabXD.setindex(dims, 1, i)
        end
    end
...
function _mapreduce_order!(@nospecialize(f), @nospecialize(op), @nospecialize(initop),
    ...
    p = TupleLabXD.sortperm(importance, rev = true)
    dims = TupleLabXD.getindices(dims, p)
    strides = broadcast(TupleLabXD.getindices, strides, (p,))
...
function _computeblocks(dims::NTuple{N,Int}, costs::NTuple{N,Int},
    ...
    while totalmemoryregion(blocks, bytestrides) >= 2*blocksize
        ...
        blocks = TupleLabXD.setindex(blocks, (blocks[i]+1)>>1, i)
    end
    while totalmemoryregion(blocks, bytestrides) > blocksize
        i = _lastargmax((blocks .- 1) .* costs)
        blocks = TupleLabXD.setindex(blocks, blocks[i]-1, i)
    end
    return blocks
end
